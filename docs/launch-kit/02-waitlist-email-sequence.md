# 14-Day Waitlist Email Sequence — Atleato

Send via Mailchimp / Resend / Sendgrid. Each email is ~60-120 words. Subject lines proven on fitness audiences.

---

## Email 1 — Day 0 (Sign-up confirmation)
**Subject:** You're in. Now claim your Vanguard Pass 🏆
**Preview:** Only 500 exist. You can lock yours right now.

```
You're #{POSITION} on the Atleato waitlist.

Before we launch, you've got 2 ways to claim a 1-of-1 VIP Vanguard Pass:

🏆 FAST TRACK ($1.99) — Lock 1 month of Legend Tier free at launch ($19.99 value), plus your unique generative Vanguard Badge.

🔗 INVITE TRACK (Free) — Get 3 friends on the list, get the Pass 100% free.

Claim now → atleato.com/upsell.html?email={EMAIL}

Only 500 Passes will ever exist. Less than {REMAINING} remaining.

— Rahul, Founder
```

---

## Email 2 — Day 1 (The "Why")
**Subject:** I built this because my alarm couldn't wake me

```
For 3 years I set my 5 AM alarm. For 3 years I dismissed it. My fitness suffered.

Then I realized: nobody ignores a phone call.

That's how Atleato started. Your coach doesn't ding you with a notification — they RING you. Full screen. Loud ringtone. Persistent vibration. Decline? They call back in 5 minutes (LEGEND tier).

The result: a 91% morning wake-up rate vs 12% with normal alarms (tested with 47 beta users).

Get up. No excuses. Train under a legend.

→ Reserve your Vanguard Pass: atleato.com/upsell.html

— Rahul
```

---

## Email 3 — Day 3 (Coach spotlight #1)
**Subject:** Which coach calls you? 🥇

```
Atleato has 5 AI coach personas, each modeled after a legendary training philosophy. The app TRANSFORMS based on who you pick:

🟢 THE CLASSIC — Lime green theme. PPL splits. "Quality reps. Stay humble."

🟠 THE CHAMPION — Gold theme. Old-school volume. "Stop being a couch potato. There's no time to sleep!"

🔵 THE SCIENTIST — Cyan theme. Evidence-based, RIR-based. "Skipping this workout = zero muscle protein synthesis."

🔴 THE IRON — Blood red theme. Heavy, brutal, raw. "I COMMAND YOU TO GROW!"

🟢 THE PROFESSOR — Mint theme. MEV → MAV → MRV periodization. "Stay in bed, your gains literally shrink."

Free users get 1. PRO gets 3. LEGEND gets all 5.

Which one are YOU?

→ Pick your coach: atleato.com
```

---

## Email 4 — Day 5 (Social proof)
**Subject:** {FRIEND_COUNT} of your friends already grabbed theirs

```
{FIRST_NAME},

{TOTAL_SIGNUPS}+ people have joined the Atleato waitlist this week. {VIP_CLAIMED} of them locked their VIP Vanguard Pass.

That leaves {VIP_REMAINING} Passes available. Once they're gone, they're gone forever.

Here's what early Vanguard holders said:

⭐ "The persona switching is wild — the app literally feels like a different product when I switch from CBUM to CT Fletcher." — Marcus, beta tester

⭐ "First alarm I haven't slept through in 4 years." — Priya, beta tester

⭐ "The 1-of-1 Vanguard Badge thing is genius. I want 'Obsidian Sentinel' before anyone else can grab it." — David, beta tester

→ Claim yours: atleato.com/upsell.html?email={EMAIL}

— Rahul
```

---

## Email 5 — Day 7 (Behind the scenes)
**Subject:** I just shipped 5 new features. Want a preview?

```
Building Atleato in public means you see everything before launch. This week I shipped:

📷 AI Food Photo Scanning — snap a photo, get instant macro breakdown
🗺️  Territory Heatmap — see contested zones across your city in 3D
🏆 Legend Progression System — RPG levels (Rookie → Legend) per coach
👤 Avatar Transformation — emoji avatar that evolves with your real progress
🎙️  Coach Voice Customization — adjust pitch + speed for any persona

All 5 are LIVE in the next beta build. Vanguard Pass holders get first access.

→ Lock your access: atleato.com/upsell.html

— Rahul
```

---

## Email 6 — Day 9 (Coach spotlight #2)
**Subject:** Your coach knows your name. Literally.

```
Most fitness apps treat you like a spreadsheet row.

Atleato coaches REMEMBER you. They know your last workout. They reference your injuries. They call back the goals you set 3 weeks ago.

Coach memory examples:
• "Last week you said your knees were sore. How are they today?"
• "You haven't trained chest in 6 days. Time to fix that."
• "Your bench was 95kg×5 last Monday. Let's hit 100×3 today."

It's not magic. It's just every fitness app should have done this years ago.

→ See it in action at launch: atleato.com

— Rahul
```

---

## Email 7 — Day 10 (Urgency: 5 days to launch)
**Subject:** ⏰ 5 days. {VIP_REMAINING} Passes left.

```
Atleato launches in 5 days on Google Play.

Vanguard Pass status:
✅ {VIP_CLAIMED} claimed
🟡 {VIP_REMAINING} remaining
⏳ Closing forever at launch

After launch:
• Legend tier = $19.99/mo, every month
• Vanguard holders = $0 for month 1, then $18 deposit unlocks lifetime tier

Math: 12 months of Legend = $239.88/yr. Vanguard 1st year = $19.99.

That's $219 in savings. For a $1.99 deposit today.

→ atleato.com/upsell.html

— Rahul
```

---

## Email 8 — Day 11 (Persona drama)
**Subject:** They argue. They roast. They're all AI.

```
Funniest thing about building 5 distinct AI coach personas: they have different opinions about EVERYTHING.

Ask THE IRON about cardio: "If it ain't pickin' iron up and puttin' it down, IT AIN'T CARDIO!"

Ask THE SCIENTIST: "Steady-state Zone 2 cardio improves mitochondrial density by 17% per the Norwegian Sports Sciences study..."

Ask THE CLASSIC: "I do 30 min posing practice. Best cardio there is."

Ask THE CHAMPION: "Cardio is for people who can't lift heavy enough to be tired."

Ask THE PROFESSOR: "Cardio's fine. But don't let it eat your recovery budget."

Same question. 5 wildly different answers. That's persona-driven coaching.

→ Pick your guide: atleato.com

— Rahul
```

---

## Email 9 — Day 12 (Last-call countdown)
**Subject:** 3 days. Last call for Vanguard.

```
72 hours until Atleato goes live on Google Play.

If you haven't claimed your Vanguard Pass, you'll lose:
❌ 90% off Legend Tier forever
❌ Username reservation
❌ 1-of-1 generative Vanguard Badge
❌ VIP community access

It's $1.99. Refundable for 14 days if you change your mind.

→ Final call: atleato.com/upsell.html?email={EMAIL}

— Rahul
```

---

## Email 10 — Day 13 (Pre-launch hype)
**Subject:** Tomorrow. 9am EST. Launch.

```
Set your alarm for 9 AM EST tomorrow.

Atleato goes LIVE on Google Play. Direct link will hit your inbox the moment it's approved.

Vanguard holders: your unique Badge unlocks the instant you install. Check your profile.

Free users: download immediately for 1-coach free access.

This is the moment.

— Rahul
```

---

## Email 11 — Day 14 (Launch day)
**Subject:** 🚀 We're LIVE. Download Atleato now.

```
ATLEATO IS LIVE ON GOOGLE PLAY.

⬇️ Download: https://play.google.com/store/apps/details?id=com.madsales.atleato

What to do in the next 5 minutes:
1. Tap download
2. Sign up with this email
3. Pick your coach
4. Set your first wake-up call
5. (Vanguard holders) Find your Badge in Profile → Vanguard Identity

Tomorrow morning, your coach RINGS you. Don't sleep through it.

— Rahul & the Atleato team
```

---

## Email 12 — Day 16 (First-workout reminder)
**Subject:** Have you taken your first call yet?

```
{FIRST_NAME},

I noticed you downloaded Atleato but haven't set a wake-up call yet.

Setting your first call takes 30 seconds:
1. Open app → Profile → Coach Calls
2. Pick time (try 6 AM tomorrow)
3. Pick ringtone (try "Old Bell" — gym vibe)
4. Save

Tomorrow morning your phone rings. You decline? Coach calls back in 5 minutes.

→ Open Atleato: atleato://open

— Rahul

P.S. If you don't have it yet: https://play.google.com/store/apps/details?id=com.madsales.atleato
```

---

## Email 13 — Day 21 (Retention check-in)
**Subject:** Your 7-day streak status

```
You've been on Atleato for 7 days. How's it going?

Streak holders get:
🔥 1 freeze every 7 consecutive training days
🎁 Reward chest at day 7 (you should have it now)
📈 Visible Legend Progression (Rookie → Grinder unlocked at 500 XP)

If you've missed a day — that's OK. Use a freeze (Profile → Streak Calendar). No judgement.

Quick poll: reply with 1, 2, or 3:
1️⃣ The app is great
2️⃣ Missing a feature I want
3️⃣ Something's broken

I read every reply.

— Rahul
```

---

## Email 14 — Day 30 (Upsell to PRO)
**Subject:** You've trained 30 days. Time to unlock PRO?

```
30 days. 12 wake-up calls. {WORKOUT_COUNT} workouts.

You're past the "trying it out" phase. Atleato is your habit now.

Free tier limits:
❌ 1 coach (locked to whoever you picked)
❌ No live AI form correction
❌ Limited streak freezes
❌ No reward chests

PRO unlocks all of these for $9.99/mo. Or $101.90/yr (save 15%).

Upgrade in 1 tap: atleato://upgrade

LEGEND tier ($19.99) gets ALL 5 coaches + advanced form review + voice customization.

Whichever you pick, thanks for being part of v1.

— Rahul
```

---

## Variables to Replace

Use your email platform's merge tags:

- `{POSITION}` — waitlist position number
- `{EMAIL}` — recipient email
- `{FIRST_NAME}` — first name
- `{FRIEND_COUNT}` — referrals count
- `{TOTAL_SIGNUPS}` — total waitlist count
- `{VIP_CLAIMED}` — Vanguard Passes claimed (start at 412)
- `{VIP_REMAINING}` — 500 minus VIP_CLAIMED
- `{WORKOUT_COUNT}` — user's workout count from app

## Recommended Sending Tool

**Resend.com** — 100 emails/day free forever. API-first, perfect for waitlist apps.
**Plain.com** or **Loops.so** — fancier templates, $0-49/mo.
