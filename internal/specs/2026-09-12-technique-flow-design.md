...
**Batch 2 — technique cards** (`constants/techniqueCards.ts`, 2026-09-14). The 14 `ExerciseForm` entries cover a fraction of what a user can tap. Every other exercise in `EXERCISE_LIBRARY` and `EXPERT_PROGRAMS` (58 names, per the batch catalog) now has a `TechniqueCard`: the lighter sibling of an `ExerciseForm` — stable `id` (clip stem + memory key), exact-name `aliases`, a `posture` for the setup figure, `cameraAngle`/`cameraNote`, 3–5 `keyPoints` reusing the exercise's own `tips`, and an explicit `visionCategory` with `detectedFaults` only when it is non-null. No joint-angle table, no coach cues: a card never adds a biomechanics claim the engine does not make. `getOwnTechnique(name)` in `exerciseFormLibrary.ts` projects either table onto one `OwnTechnique` shape (form entry first, then card; exact title or alias only), and the screens read that: `technique.tsx` derives clip path, figure orientation (`lying`/`prone` → lying) and camera note from it, `form-coach.tsx` resolves the engine category with `visionCategoryForName` and re-trigger labels with `detectedFaultsFor`. **Honesty rule for `visionCategory`:** a card's decision is made per exercise against what the profile can actually measure and wins BY NAME over the keyword family match — including an explicit `null`. Hack Squat's keyword would route it to the squat profile; its card says `null` (the depth check is meaningless on a sled), so it gets a how-to card and no camera. `hasVisionCoverage`, `getExerciseFormKey` and `keyPointsFor` all resolve through `getOwnTechnique`, so a name with a card is OWN content (its own clip, key points and memory key), never a "clip coming soon" variant of its family. Tests: `__tests__/techniqueCards.test.ts` pins all 58 rows; `__tests__/exerciseFormLibrary.test.ts` asserts every library/program name resolves to an own technique and prints the misses.
...
- **URL:** `<EXPO_PUBLIC_SUPABASE_URL>/storage/v1/object/public/tutorial-clips/<form.id>_v<version>.mp4`, built in `lib/tutorialClips.ts`. **Looked up by convention for every exercise with a form entry** — `version` is 1 unless the entry's `tutorial` says otherwise — so uploading a file is the whole release. A 404 is remembered for one hour (`tutorial_clips:missing:v1`) so cards without a clip open straight to the poster; that hour is the longest a new upload takes to appear.
- **Versioned filenames** (`bench_press_v1.mp4`) because public objects are CDN-cached; overwriting in place serves stale bytes.
- **Manifest (`manifest.json`, same bucket, public):** `{"version":1,"clips":{"<form.id>":<version>}}`. `resolveClipVersion(id, entry.tutorialVersion)` in `lib/tutorialClips.ts` returns `max(manifest[id] ?? 0, entry.tutorialVersion)`, so replacing a clip (illustrated → licensed footage) is an upload of `<id>_v<n+1>.mp4` plus a manifest edit — no release. **Forward only:** the manifest can raise a clip's version, never lower it below the one compiled into the app, so a stale CDN copy or a typo can never point a newer build at an older (possibly removed) object. Cached for one hour (`tutorial_clips:manifest:v1`, memory + AsyncStorage); fetched with a 4 s timeout when stale, and any failure (offline, non-200, bad JSON, missing env) falls back to the last good copy or `{}` — never throws, never blocks the poster. `objectPath` on the technique screen is `null` (poster) until the version is resolved.
